...
interface BookSourceDao {
    ...
    @get:Query(
        """select bp.*
        from book_sources b join book_sources_part bp on b.bookSourceUrl = bp.bookSourceUrl
        where b.enabled = 1 and b.bookSourceType = 2 order by b.customOrder"""
    )
    val allImageEnabledPart: List<BookSourcePart>
    ...
    @Query("""update book_sources set respondTime = :elapsedMs,
        weight = case when weight < 100 then weight + 1 else weight end
        where bookSourceUrl = :sourceUrl""")
    fun recordSearchSuccess(sourceUrl: String, elapsedMs: Long)

    @Query("""update book_sources set respondTime = 180000,
        weight = case when weight > -100 then weight - 2 else weight end
        where bookSourceUrl = :sourceUrl""")
    fun recordSearchFailure(sourceUrl: String)

    @Query("""update book_sources set respondTime = :elapsedMs,
        weight = case when weight < 100 then weight + 1 else weight end
        where bookSourceUrl = :sourceUrl and bookSourceType = 2""")
    fun recordVideoSuccess(sourceUrl: String, elapsedMs: Long)

    @Query("""update book_sources set respondTime = 180000,
        weight = case when weight > -100 then weight - 2 else weight end
        where bookSourceUrl = :sourceUrl and bookSourceType = 2""")
    fun recordVideoFailure(sourceUrl: String)
