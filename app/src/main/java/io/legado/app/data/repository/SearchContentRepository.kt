...
class SearchContentRepository {
    ...
    private fun searchKey(
        ...
        // Unit tests and early process startup may run before Koin is initialized.
        // Falling back only affects the in-memory cache key for that short window.
        val titleMode = runCatching { ReadBookConfig.titleMode }.getOrDefault(0)
        return "$bookUrl-$query-$replaceEnabled-$regexReplace-$titleMode"
    }
