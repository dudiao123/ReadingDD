...
object DefaultData {
    ...
    private val bundledVideoSources: List<BookSource> by lazy {
        val json = String(
            appCtx.assets.open("defaultData${File.separator}videoSources.json").readBytes()
        )
        GSON.fromJsonArray<BookSource>(json).getOrDefault(emptyList())
    }

    private fun importBundledVideoSources() {
        bundledVideoSources
            .filterNot { appDb.bookSourceDao.has(it.bookSourceUrl) }
            .forEach { appDb.bookSourceDao.insert(it) }
    }
    ...
    val keyboardAssists: List<KeyboardAssist> by lazy {
        val json = String(
            appCtx.assets.open("defaultData/keyboardAssists.json")
                .readBytes()
        )
