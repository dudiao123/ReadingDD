...
object SourceVerificationHelp {
    ...
    private val nonInteractiveSources =
        Collections.synchronizedMap(IdentityHashMap<BaseSource, AtomicBoolean>())

    suspend fun <T> withoutInteraction(source: BaseSource, block: suspend () -> T): T {
        val requested = AtomicBoolean(false)
        nonInteractiveSources[source] = requested
        try {
            val result = block()
            if (requested.get()) throw NoStackTraceException("书源需要登录或验证码")
            return result
        } finally {
            nonInteractiveSources.remove(source)
        }
    }

    private fun requireInteraction(source: BaseSource) {
        nonInteractiveSources[source]?.let {
            it.set(true)
            throw NoStackTraceException("书源需要登录或验证码")
        }
    }
