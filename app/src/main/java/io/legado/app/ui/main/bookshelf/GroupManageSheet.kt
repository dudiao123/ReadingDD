...
fun GroupManageSheet(
    ...
    // 顶层板块和系统状态组由书架统一管理，这里只显示用户自定义分组。
    val customGroups = remember(groups, bookshelfState.visibleGroupBooks) {
        groups.filter {
            it.groupId > 0 && bookshelfState.visibleGroupBooks[it.groupId]?.isNotEmpty() == true
        }
    }
    ...
    var listData by remember { mutableStateOf(customGroups) }
    ...
    LaunchedEffect(customGroups) {
        if (!reorderableState.isAnyItemDragging) {
            listData = customGroups
        }
    }
