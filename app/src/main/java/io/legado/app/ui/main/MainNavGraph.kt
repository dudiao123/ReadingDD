...
import io.legado.app.ui.main.bookshelf.BookshelfSection
...
fun MainActivity.mainEntryProvider(
    ...
    entry<MainRouteHome> {
        MainScreen(
            ...
            onNavigateToSearch = { key, section ->
                val sectionGroup = when (section) {
                    BookshelfSection.Novel -> "小说"
                    BookshelfSection.Manga -> "漫画"
                    BookshelfSection.Anime -> "动漫"
                    null -> null
                    else -> null
                }
                onNavigateToRoute(
                    MainRouteSearch(
                        ...
                        scopeRaw = sectionGroup?.let { SearchScope(listOf(it)).toString() },
                    )
                )
            },
    ...
    entry<MainRouteSearch> { route ->
        ...
        val bookshelfState = bookshelfViewModel.uiState.collectAsStateWithLifecycle().value
        val sectionGroup = when (bookshelfState.selectedSection) {
            BookshelfSection.Novel -> "小说"
            BookshelfSection.Manga -> "漫画"
            BookshelfSection.Anime -> "动漫"
            else -> "小说"
        ...
        LaunchedEffect(route.key, route.scopeRaw, searchViewModel) {
            searchViewModel.onIntent(
                SearchIntent.Initialize(
                    key = route.key,
                    scopeRaw = route.scopeRaw ?: SearchScope(listOf(sectionGroup)).toString()
                )
            )
        }
