...
class ImportBookSourceDialog() : BaseBottomSheetDialogFragment(R.layout.dialog_recycler_view),
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.checkResultChanged.observe(viewLifecycleOwner) { index ->
            if (index >= 0) adapter.notifyItemChanged(index)
            updateCheckProgress()
        }
        viewModel.checkProgress.observe(viewLifecycleOwner) { updateCheckProgress() }
        viewModel.checking.observe(viewLifecycleOwner) { checking ->
            binding.tvOk.isEnabled = !checking
            binding.toolBar.menu.findItem(R.id.menu_check_import_sources).setTitle(
                if (checking) R.string.import_check_stop else R.string.import_check_selected
            )
            binding.toolBar.menu.findItem(R.id.menu_select_passed_sources).isEnabled = !checking
            adapter.setItems(viewModel.allSources)
            upSelectText()
            updateCheckProgress()
        }
    ...
    private fun initMenu() {
        ...
        binding.toolBar.inflateMenu(R.menu.import_book_source_check)
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.menu_check_import_sources -> {
                if (viewModel.checking.value == true) {
                    viewModel.stopChecking()
                } else if (viewModel.selectCount == 0) {
                    toastOnUi(R.string.import_check_select_first)
                } else {
                    alert(R.string.import_check_selected) {
                        val input = DialogEditTextBinding.inflate(layoutInflater)
                        input.editView.setHint(R.string.import_check_hint)
                        input.editView.setText(CheckSource.keyword)
                        customView { input.root }
                        okButton {
                            val keyword = input.editView.text?.toString()?.trim()
                                .orEmpty().ifBlank { CheckSource.keyword }
                            viewModel.checkSelected(keyword)
                        }
                        cancelButton()
                    }
                }
            ...
            R.id.menu_select_passed_sources -> {
                viewModel.selectPassed()
                adapter.notifyDataSetChanged()
                upSelectText()
                return true
            }
            R.id.menu_import_check_report -> {
                showDialogFragment(TextDialog(
                    getString(R.string.import_check_report), viewModel.checkReport.joinToString("\n\n")
                ))
                return true
            }
    ...
    private fun updateCheckProgress() {
        val (completed, total) = viewModel.checkProgress.value ?: return
        binding.toolBar.subtitle = getString(
            R.string.import_check_progress, completed, total, viewModel.passedCount
        )
    }
        ...
        override fun convert(
            ...
            binding.apply {
                ...
                tvSourceState.text = when {
                    viewModel.checkResults[holder.layoutPosition]?.first == true ->
                        getString(R.string.import_check_passed)
                    viewModel.checkResults[holder.layoutPosition]?.first == false ->
                        getString(R.string.import_check_failed)
                    localSource == null -> "新增"
                    item.lastUpdateTime > localSource.lastUpdateTime -> "更新"
                    else -> "已有"
                }
                tvOpen.isEnabled = viewModel.checking.value != true
            }
        }
