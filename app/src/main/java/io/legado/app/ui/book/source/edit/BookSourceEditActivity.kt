...
class BookSourceEditActivity :
    ...
    override fun onCompatOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.menu_check_javascript_syntax -> checkFocusedJavaScript()

        }
        return super.onCompatOptionsItemSelected(item)
    }

    private fun checkFocusedJavaScript() {
        val editor = currentFocus as? EditText
        val source = editor?.text?.toString().orEmpty()
        val scripts = buildList {
            listOf(AppPattern.JS_PATTERN, AppPattern.EXP_PATTERN).forEach { pattern ->
                val matcher = pattern.matcher(source)
                while (matcher.find()) {
                    val group = (1..matcher.groupCount()).firstOrNull {
                        matcher.start(it) >= 0 && !matcher.group(it).isNullOrBlank()
                    } ?: continue
                    add(JavaScriptBlock(matcher.group(group).orEmpty(), matcher.start(group)))
                }
            }
        }.distinctBy { it.code to it.offset }
        if (scripts.isEmpty()) {
            toastOnUi(R.string.javascript_not_found)
            return
        }
        lifecycleScope.launch(IO) {
            val failure = scripts.firstNotNullOfOrNull { block ->
                runCatching { RhinoScriptEngine.compile(block.code) }
                    .exceptionOrNull()
                    ?.let { block to it }
            }
            withContext(kotlinx.coroutines.Dispatchers.Main) {
                if (failure == null) {
                    toastOnUi(getString(R.string.javascript_syntax_correct, scripts.size))
                } else {
                    val (block, error) = failure
                    val scriptError = error as? ScriptException
                    val lineOffset = scriptError?.lineNumber?.takeIf { it > 0 }?.let { line ->
                        block.code.lineSequence().take(line - 1).sumOf { it.length + 1 }
                    } ?: 0
                    editor?.setSelection((block.offset + lineOffset).coerceIn(0, source.length))
                    editor?.requestFocus()
                    toastOnUi(getString(
                        R.string.javascript_syntax_error,
    ...
    private data class JavaScriptBlock(val code: String, val offset: Int)
